...
> 4. **AI 聊天不能直接写正式健康记录**：Agent 只读 + 生成修改草案，用户确认后经 health-card-confirm 落库；
...
| 2 | 健康卡片草案 Schema | draftId / items / operation / before-after / confidence |
| 3 | health-card-confirm | 请求/响应、Idempotency-Key、事务落库 |
...
| 6 | 对话中口头确认（"对"）→ 自动提交 confirm | MVP 可后置，先走弹窗确认 |
