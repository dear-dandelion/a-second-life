...
| AI 聊天 Agent | 符合 | 文字/语音、RAG、健康草案自动创建、受控工具调用 |
| 两类用户 | 符合 | 本人用户有健康记录/报告；支持者用户无这两项能力 |
| 好友系统 | 不进入 MVP | 好友列表、交友入口和消息提醒均为前端假数据；所有交互 toast“敬请期待” |
| 健康报告 | 符合 | 自动生成可编辑预览、本地 PDF、系统下载/分享 |

这里的“自动记录创建”定义为：AI 自动创建一条**待确认健康记录草案**，而不是静默写入正式健康档案。用户确认后才转成正式记录。这既符合原 PRD 的“弹窗预览、逐条勾选保存”，也能降低 ASR 或模型误识别造成的健康数据污染。
...
| 絮絮叨叨 | 文字/单句语音、TTS、SSE、多轮 Agent、最小 RAG、安全提示、10 类健康卡片草案及确认、已实现页面导航 | 实时 ASR 字幕、持续监听、未实现页面工具 |
...
| `POST /api/health-card/confirm` | 确认草案并事务性写入健康记录 |
...
- 对话上下文中的健康信息只用于回答，不等同于正式健康记录；正式事实只来自用户确认后的记录。
...
- 后端 API 成功率 ≥ 99.5%，且不存在 API Key 前端泄露和未确认写入健康记录的问题。
