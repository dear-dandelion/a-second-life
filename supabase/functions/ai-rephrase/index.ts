...
interface RephraseRequest { clientRequestId: string; text: string; style: "warm" }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!input || input.length > 2000 || !body.clientRequestId || body.clientRequestId.length > 128 || body.style !== "warm") {
      throw new ApiError("VALIDATION_ERROR", "请输入需要整理的内容", 400);
    }
    const requestHash = await sha256({ text: input, style: body.style });
    ...
    let output: string;
    if (isMockMode()) {
      output = `我想认真和你说说我的感受：${input}。如果你方便，我希望你能听我慢慢说，也谢谢你的理解。`;
    } else {
      if (!hasTextModel()) throw new ApiError("MODEL_NOT_CONFIGURED", "模型服务暂未配置", 503);
      output = (await completeText([
        { role: "system", content: "把用户原话整理成温和、真诚、不指责的中文表达。保留事实和诉求，不添加新事实，只输出改写后的正文。" },
