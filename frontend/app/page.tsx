...
function monthPoint(day:MonthDay,dimension:string):MonthPoint{
  ...
  if(dimension==='我的潮热'){const value=Number(day.hotFlash?.chartValue)||0;return{value:value||null,marker:value>=3?'heavy':value>=2?'medium':'light',label:value?`潮热 ${value} 次`:'暂无潮热记录'}}
  if(dimension==='我的心情'){const state=String(day.mood?.state??'');if(!state)return{value:null,marker:'',label:'暂无心情记录'};const value=Number(day.mood?.chartValue)||0;const markers:Record<string,string>={'舒展':'mood-relaxed','平静':'mood-calm','低落':'mood-low','焦虑':'mood-anxious','烦躁':'mood-irritable','复杂':'mood-complex'};return{value,marker:markers[state]??'mood-complex',label:`心情：${state}${day.mood?.intensity?`（${String(day.mood.intensity)}）`:''}`}}
...
function Trend({dimension,days}:{dimension:string;days:MonthlyHealthStats['days']}){
  ...
  const max=dimension==='我的睡眠'?600:dimension==='我的运动'?Math.max(60,Math.ceil(Math.max(...numeric,0)/30)*30):dimension==='我的心情'?3:Math.max(3,...numeric);
  const min=dimension==='我的心情'?-3:0;
  ...
  const zeroY=y(0);
  const axis=dimension==='我的睡眠'?['10小时','5小时','0小时']:dimension==='我的运动'?[durationLabel(max),durationLabel(Math.round(max/2)),'0分']:dimension==='我的心情'?['舒展','平静','低落']:['高','中','0'];
  return <div className="trend-panel"><div className="trend-graph"><div className="trend-y-labels">{axis.map((label,index)=><span key={index}>{label}</span>)}</div><svg viewBox="0 0 320 150" aria-label={`${dimension}月度柱状图`}><line className="bar-zero-line" x1="8" x2="314" y1={zeroY} y2={zeroY}/>{values.map((point,index)=>{if(point.value===null||(point.value===0&&point.marker!=='mood-complex'))return null;const valueY=y(point.value);const top=point.value===0?zeroY-2:Math.min(valueY,zeroY);return <rect className={`bar-fill ${point.marker}`} key={index} x={x(index)} y={top} width={width} height={point.value===0?4:Math.max(2,Math.abs(zeroY-valueY))} rx="2"><title>{point.label}</title></rect>})}</svg></div><div className="trend-labels"><span>1日</span><span>8日</span><span>15日</span><span>22日</span><span>月底</span></div><p>{numeric.some(Boolean)?'虚线为 0 基准；标注“缺乏具体数据”的柱为估算值':'本月暂无可绘制数据'}</p></div>
}
