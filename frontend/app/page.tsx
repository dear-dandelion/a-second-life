...
import {
  ArrowLeft, ArrowRight, Award, BarChart3 as Waves, Bone, CalendarDays, Check, ChevronRight, CircleUserRound,
  ...
  UserRound, UsersRound, Utensils, Volume2, X,
} from 'lucide-react';
...
type ProfileData = { birth: string; height: string; menopausalStatus: ''|'未绝经'|'围绝经期'|'绝经后'|'不确定'; history: string; surgery: string; allergies: string; regularMedications: string; pregnancyHistory: string; familyHistory: string; screeningHistory: string };
...
function homeMonthlyOverview(stats:MonthlyHealthStats|null){
  ...
  const sleepStatus=durations.length>=2?(quality.values.length!==1?'睡眠有波动':quality.values[0]==='好'?'大多良好':quality.values[0]==='一般'?'整体一般':'睡得不稳'):sleeps.length&&durations.length===0?'时长待补':sleeps.length?'正在记录':'今晚记睡眠';
  ...
  const exerciseStatus=exerciseDays>=2&&exerciseDurations.length>=2?`累计${Math.round(exerciseDurations.reduce((sum,value)=>sum+value,0)/60)}时`:exerciseDays>=2?'正在记录':exerciseDays===1?'保持节奏':'去动一动';
...
function healthItemSummary(item:HealthDraftItem){
  ...
  if(item.category==='mood')return`心情${displayValue(data.label??data.state,'已更新')}`;
...
export default function App(){
  ...
  const [profile,setProfile]=useState<ProfileData>({birth:'1978',height:'165',menopausalStatus:'围绝经期',history:'高血压',surgery:'无',allergies:'无',regularMedications:'',pregnancyHistory:'',familyHistory:'',screeningHistory:''});
  ...
  useEffect(()=>{services.profile.get().then(value=>setProfile({birth:String(value.birthYear??''),height:value.heightCm===null?'':String(value.heightCm),menopausalStatus:value.menopausalStatus,history:value.medicalHistory,surgery:value.surgeryHistory,allergies:value.allergyHistory,regularMedications:value.regularMedications,pregnancyHistory:value.pregnancyHistory,familyHistory:value.familyHistory,screeningHistory:value.screeningHistory})).catch(()=>undefined)},[]);
...
function Month({back,go,dimension,setDimension,chart,setChart,onEditHealth}:{back:()=>void;go:(v:View)=>void;dimension:string;setDimension:(v:string)=>void;chart:boolean;setChart:(v:boolean)=>void;onEditHealth:()=>void}){
  ...
  useEffect(()=>{let active=true;services.healthRecords.monthly(month).then(value=>{if(active)setStats(value)}).catch(()=>{if(active){setStats(null);toast.add({title:'月度记录读取失败',type:'error'})}});return()=>{active=false}},[month]);
  const move=(offset:number)=>{const [year,value]=month.split('-').map(Number);setMonth(new Date(Date.UTC(year,value-1+offset,1)).toISOString().slice(0,7))};
  const days=stats?.days??[];
  const recorded=days.filter(day=>day.hasRecord).length;
  const settled=days.filter(day=>['舒展','平静'].includes(String(day.mood?.state??''))).length;
  const flashes=days.reduce((sum,day)=>sum+(Number(day.hotFlash?.frequencyCount)||0),0);
  const exercise=days.filter(day=>day.exercise).length;
  const digest=recorded?`本月已记录 ${recorded} 天。多记录一点，多了解自己一点。`:'这个月还没有记录';
  return <><PageHeader title="这个月的我" onBack={back} action={<><button className="icon-button" onClick={onEditHealth} aria-label="编辑健康卡片"><Pencil/></button><button className="view-toggle" onClick={()=>setChart(!chart)}>{chart?<Waves/>:<CalendarDays/>}<span>{chart?'柱状图':'日历'}</span><small>⌄</small></button></>}/><div className="month-switch"><button onClick={()=>move(-1)}>‹</button><b>{month.replace('-','年')}月</b><button onClick={()=>move(1)} disabled={month>=new Date().toISOString().slice(0,7)}>›</button></div><div className="segmented">{dims.map(d=><button key={d} className={dimension===d?'active':''} onClick={()=>setDimension(d)}>{d}</button>)}</div>{chart?<Trend dimension={dimension} days={days}/>:<Calendar dimension={dimension} month={month} days={days}/>}<section className="month-summary monthly-bg"><h3>{digest}</h3><div><span><small>已记录</small><b>{recorded}天</b></span><span><small>舒展/平静</small><b>{settled}天</b></span><span><small>潮热</small><b>{flashes}次</b></span><span><small>运动</small><b>{exercise}天</b></span></div></section><div className="month-links"><button onClick={()=>go('summaries')}><FileText/>月度总结</button><button onClick={()=>go('range')}><ClipboardPlus/>就医报告</button></div></>}
type MonthDay=MonthlyHealthStats['days'][number];
type MonthPoint={value:number|null;marker:string;label:string;estimated?:boolean};
const sleepEstimate=(quality:unknown)=>quality==='好'?450:quality==='差'?330:390;
const durationLabel=(minutes:number)=>`${Math.floor(minutes/60)}小时${minutes%60?`${minutes%60}分`:''}`;
function exerciseMinutes(value:unknown){const text=String(value??'').replace(/\s/g,'');const hours=[...text.matchAll(/(\d+(?:\.\d+)?)小时/g)].reduce((sum,item)=>sum+Number(item[1])*60,0);const minutes=[...text.matchAll(/(\d+(?:\.\d+)?)(?:分钟|分|min)/gi)].reduce((sum,item)=>sum+Number(item[1]),0);return hours+minutes||null}
function monthPoint(day:MonthDay,dimension:string):MonthPoint{
  if(dimension==='我的睡眠'){const sleep=day.sleep;if(!sleep)return{value:null,marker:'',label:'暂无睡眠记录'};const actual=Number(sleep.durationMinutes);const estimated=!Number.isFinite(actual)||actual<=0;const value=estimated?sleepEstimate(sleep.quality):actual;const marker=sleep.quality==='好'?'sleep-good':sleep.quality==='差'?'sleep-poor':'sleep-average';return{value,marker,label:`睡眠 ${durationLabel(value)}${estimated?'（估算，缺乏具体数据）':''}`,estimated}}
  if(dimension==='我的潮热'){if(!day.hotFlash)return{value:null,marker:'',label:'暂无潮热记录'};const raw=Number(day.hotFlash.chartValue);const value=Number.isFinite(raw)?raw:0;return{value,marker:value>=3?'heavy':value>=2?'medium':'light',label:`潮热 ${value} 次`}}
  if(dimension==='我的心情'){const state=String(day.mood?.state??'');if(!state)return{value:0,marker:'',label:'暂无心情记录'};const values:Record<string,number>={'低落':1,'焦虑':2,'烦躁':3,'复杂':4,'平静':5,'舒展':6};const markers:Record<string,string>={'舒展':'mood-relaxed','平静':'mood-calm','低落':'mood-low','焦虑':'mood-anxious','烦躁':'mood-irritable','复杂':'mood-complex'};return{value:values[state]??0,marker:markers[state]??'mood-complex',label:`心情：${state}${day.mood?.intensity?`（${String(day.mood.intensity)}）`:''}`}}
  if(dimension==='我的运动'){if(!day.exercise)return{value:0,marker:'',label:'无运动记录'};const actual=exerciseMinutes(day.exercise.duration);const estimated=actual===null;const value=actual??30;return{value,marker:'exercise',label:`运动 ${durationLabel(value)}${estimated?'（估算，缺乏具体数据）':''}`,estimated}}
  return{value:null,marker:'',label:'暂无记录'};
}
function monthLegend(dimension:string){if(dimension==='我的睡眠')return <><i className="sleep-good"/>好　<i className="sleep-average"/>一般　<i className="sleep-poor"/>差</>;if(dimension==='我的心情')return <><i className="mood-relaxed"/>舒展　<i className="mood-calm"/>平静　<i className="mood-low"/>低落　<i className="mood-anxious"/>焦虑　<i className="mood-irritable"/>烦躁　<i className="mood-complex"/>说不清</>;if(dimension==='我的运动')return <><i className="exercise"/>今天有运动</>;return <><i className="light"/>轻　<i className="medium"/>中　<i className="heavy"/>重</>}
function Calendar({dimension,month,days}:{dimension:string;month:string;days:MonthlyHealthStats['days']}){const [year,value]=month.split('-').map(Number);const offset=(new Date(year,value-1,1).getDay()+6)%7;return <div className="calendar"><div className="week">{['一','二','三','四','五','六','日'].map(x=><b key={x}>{x}</b>)}</div><div className="days">{Array.from({length:offset},(_,index)=><span key={`blank-${index}`}/>)}{days.map(day=>{const point=monthPoint(day,dimension);return <button key={day.date} title={point.value===null?(day.hasRecord?'该维度暂无记录':'暂无记录'):point.label}><span>{Number(day.date.slice(-2))}</span>{point.marker&&<i className={point.marker}/>}</button>})}</div><div className="legend">当前查看：{dimension}　{monthLegend(dimension)}</div></div>}
function Trend({dimension,days}:{dimension:string;days:MonthlyHealthStats['days']}){
  const values=days.map(day=>monthPoint(day,dimension));
  const numeric=values.flatMap(point=>point.value===null?[]:[point.value]);
  const max=dimension==='我的睡眠'?600:dimension==='我的运动'?Math.max(60,Math.ceil(Math.max(...numeric,0)/30)*30):dimension==='我的心情'?6:Math.max(3,...numeric);
  const min=0;
  ...
  const zeroY=y(0);
  const axis=dimension==='我的睡眠'?['10小时','5小时','0小时']:dimension==='我的运动'?[durationLabel(max),durationLabel(Math.round(max/2)),'0分']:['高','中','0'];
  const moodAxisTop=(value:number)=>`${Math.round(y(value)/150*170-5)}px`;
  return <div className="trend-panel"><div className="trend-graph"><div className={`trend-y-labels${dimension==='我的心情'?' mood-axis':''}`}>{dimension==='我的心情'?<><span style={{top:moodAxisTop(6)}}>舒展 6</span><span style={{top:moodAxisTop(4)}}>复杂 4</span><span style={{top:moodAxisTop(1)}}>低落 1</span></>:axis.map((label,index)=><span key={index}>{label}</span>)}</div><svg viewBox="0 0 320 150" aria-label={`${dimension}月度柱状图`}><line className="bar-zero-line" x1="8" x2="314" y1={zeroY} y2={zeroY}/>{values.map((point,index)=>{if(point.value===null||point.value===0)return null;const valueY=y(point.value);const top=Math.min(valueY,zeroY);return <rect className={`bar-fill ${point.marker}`} key={index} x={x(index)} y={top} width={width} height={Math.max(2,Math.abs(zeroY-valueY))} rx="2"><title>{point.label}</title></rect>})}</svg></div><div className="trend-labels"><span>1日</span><span>8日</span><span>15日</span><span>22日</span><span>月底</span></div><p>{numeric.some(Boolean)?'虚线为 0 基准；标注“缺乏具体数据”的柱为估算值':'本月暂无可绘制数据'}</p></div>
}
...
function Report({back,profile,setProfile,soon}:{back:()=>void;profile:ProfileData;setProfile:(p:ProfileData)=>void;soon:()=>void}){const [edit,setEdit]=useState(false);const [local,setLocal]=useState(profile);const text=(value:string,fallback='待补充')=>value.trim()||fallback;const age=local.birth?new Date().getFullYear()-Number(local.birth):null;const rows=[['出生年份 / 年龄',local.birth?`${local.birth}年 / ${age}岁`:'待补充'],['身高',local.height?`${local.height}cm`:'待补充'],['绝经状态',text(local.menopausalStatus)],['长期/规律用药',text(local.regularMedications,'无记录')],['既往病史',text(local.history)],['手术史',text(local.surgery)],['药物过敏史',text(local.allergies)],['孕产与妇科手术史',text(local.pregnancyHistory)],['重要家族史',text(local.familyHistory)],['近期重要筛查',text(local.screeningHistory)]];return <><PageHeader title="就医报告预览" onBack={back} action={<button className="text-action" onClick={()=>setEdit(!edit)}>{edit?'完成':'编辑'}</button>}/><div className="report-range">统计范围　2026年3月–8月</div>{edit?<div className="edit-report"><label htmlFor="report-birth">出生年份<Input id="report-birth" value={local.birth} onChange={e=>setLocal({...local,birth:e.target.value})}/></label><label htmlFor="report-history">既往病史<Input id="report-history" value={local.history} onChange={e=>setLocal({...local,history:e.target.value})}/></label><label htmlFor="report-surgery">手术史<Input id="report-surgery" value={local.surgery} onChange={e=>setLocal({...local,surgery:e.target.value})}/></label><Button className="primary-wide" onClick={()=>{setProfile(local);setEdit(false);toast.add({title:'草稿已保存',type:'success'})}}>保存修改</Button></div>:<><section className="report-section"><h3>基础信息 · 可编辑 <Pencil/></h3>{rows.map(x=><div key={x[0]}><span>{x[0]}</span><b>{x[1]}</b><ChevronRight/></div>)}</section><section className="report-section"><h3>健康记录汇总</h3>{[['月经状态','来自健康卡片记录'],['身体症状','来自健康卡片记录'],['睡眠与情绪','来自健康卡片记录'],['运动情况','来自健康卡片记录']].map(x=><div key={x[0]}><span>{x[0]}</span><b>{x[1]}</b><ChevronRight/></div>)}</section></>}<div className="report-actions"><Button variant="outline" onClick={()=>toast.add({title:'草稿已保存',type:'success'})}>保存草稿</Button><Button onClick={()=>toast.add({title:'PDF 已生成',description:'演示版本已准备好保存或分享。',type:'success'})}>确认并生成 PDF</Button><Button variant="outline" size="icon" onClick={soon}><Share2/></Button></div></>}
...
function Personal({back,profile,prefill,setProfile}:{back:()=>void;profile:ProfileData;prefill:Record<string,string>|null;setProfile:(value:ProfileData)=>void}){
  const [draft,setDraft]=useState<ProfileData>({...profile,birth:prefill?.birthYear??profile.birth,history:prefill?.medicalHistory??profile.history,surgery:prefill?.surgeryHistory??profile.surgery});
  const [saving,setSaving]=useState(false);
  const update=(patch:Partial<ProfileData>)=>setDraft(current=>({...current,...patch}));
  const save=async()=>{
    const year=Number(draft.birth);const height=draft.height===''?null:Number(draft.height);
    if(!Number.isInteger(year)||year<1900||year>new Date().getFullYear()){toast.add({title:'请填写正确的出生年份',type:'error'});return}
    if(height!==null&&(!Number.isInteger(height)||height<80||height>250)){toast.add({title:'请填写正确的身高',type:'error'});return}
    setSaving(true);try{
      await services.profile.update({birthYear:year,heightCm:height,menopausalStatus:draft.menopausalStatus,medicalHistory:draft.history.trim(),surgeryHistory:draft.surgery.trim(),allergyHistory:draft.allergies.trim(),regularMedications:draft.regularMedications.trim(),pregnancyHistory:draft.pregnancyHistory.trim(),familyHistory:draft.familyHistory.trim(),screeningHistory:draft.screeningHistory.trim()});
      setProfile(draft);toast.add({title:'个人资料已保存',type:'success'});back();
    }catch{toast.add({title:'资料保存失败，请稍后重试',type:'error'})}finally{setSaving(false)}
  };
  return <><PageHeader title="我的资料" subtitle="用于生成更完整的就医报告" onBack={back}/>{prefill&&<div className="info-callout">小年已根据对话预填资料，请核对后保存；未保存不会写入账户。</div>}
    <section className="personal-form editable"><h3>基础信息</h3>
      <label htmlFor="profile-birth"><span>出生年份 <i>必填</i></span><Input id="profile-birth" type="number" min="1900" max={new Date().getFullYear()} value={draft.birth} onChange={e=>update({birth:e.target.value})}/></label>
      <label htmlFor="profile-height"><span>身高（cm）</span><Input id="profile-height" type="number" min="80" max="250" placeholder="如：165" value={draft.height} onChange={e=>update({height:e.target.value})}/></label>
      <label htmlFor="profile-menopause"><span>绝经阶段</span><select id="profile-menopause" value={draft.menopausalStatus} onChange={e=>update({menopausalStatus:e.target.value as ProfileData['menopausalStatus']})}><option value="">请选择</option><option>未绝经</option><option>围绝经期</option><option>绝经后</option><option>不确定</option></select></label>
    </section>
    <section className="personal-form editable"><h3>病史与安全信息</h3>
      <label htmlFor="profile-history"><span>既往病史</span><Textarea id="profile-history" placeholder="没有可填写“无”" value={draft.history} onChange={e=>update({history:e.target.value.slice(0,1000)})}/></label>
      <label htmlFor="profile-surgery"><span>手术史</span><Textarea id="profile-surgery" placeholder="没有可填写“无”" value={draft.surgery} onChange={e=>update({surgery:e.target.value.slice(0,1000)})}/></label>
      <label htmlFor="profile-allergies"><span>药物过敏史</span><Textarea id="profile-allergies" placeholder="没有可填写“无”或“不清楚”" value={draft.allergies} onChange={e=>update({allergies:e.target.value.slice(0,1000)})}/></label>
      <label htmlFor="profile-medications"><span>长期/规律用药</span><Textarea id="profile-medications" placeholder="如：药名、用途（选填）" value={draft.regularMedications} onChange={e=>update({regularMedications:e.target.value.slice(0,1000)})}/></label>
    </section>
    <section className="personal-form editable"><h3>妇科与家族背景 <small>选填</small></h3>
      <label htmlFor="profile-pregnancy"><span>孕产与妇科手术史</span><Textarea id="profile-pregnancy" placeholder="如：1次妊娠、1次分娩" value={draft.pregnancyHistory} onChange={e=>update({pregnancyHistory:e.target.value.slice(0,1000)})}/></label>
      <label htmlFor="profile-family"><span>重要家族史</span><Textarea id="profile-family" placeholder="如：母亲高血压；不清楚可不填" value={draft.familyHistory} onChange={e=>update({familyHistory:e.target.value.slice(0,1000)})}/></label>
      <label htmlFor="profile-screening"><span>近期重要筛查</span><Textarea id="profile-screening" placeholder="如：2025年乳腺超声，未见异常" value={draft.screeningHistory} onChange={e=>update({screeningHistory:e.target.value.slice(0,1000)})}/></label>
    </section>
    <p className="privacy"><LockKeyhole/>仅保存在你的账户中，你可以随时修改</p><Button className="primary-wide" disabled={saving} onClick={()=>void save()}>{saving?'保存中…':'保存资料'}</Button>
  </>}
