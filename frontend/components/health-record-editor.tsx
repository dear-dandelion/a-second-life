...
export default function HealthRecordEditor({initialDate,onBack}:{initialDate?:string;onBack:()=>void}){
  ...
  return <div className="record-editor">
    ...
    <div className="record-dimension" data-health-tab="symptoms" ref={setSectionRef('symptoms')}><Section title="身体症状" onAdd={()=>setRecord({...record,symptoms:[...record.symptoms,{symptom:'',occurred:true,quote:'用户手动填写'}]})}>
    ...
    <div className="record-dimension" data-health-tab="sleep" ref={setSectionRef('sleep')}><Section title="睡眠与作息" onClear={record.sleep?()=>setRecord({...record,sleep:undefined}):undefined} onAdd={!record.sleep?()=>setRecord({...record,sleep:{quality:'一般'}}):undefined}>
    ...
    <div className="record-dimension" data-health-tab="mood" ref={setSectionRef('mood')}><Section title="情绪" onClear={record.mood?()=>setRecord({...record,mood:undefined}):undefined}>
      <div className="record-card mood-record-card"><p className="mood-record-hint">选一个最接近今天感受的状态即可</p><div className="mood-state-options">{(['舒展','平静','低落','焦虑','烦躁','复杂'] as const).map(state=><button type="button" key={state} className={record.mood?.state===state?'active':''} onClick={()=>setRecord({...record,mood:{...record.mood,state,source:'manual'}})}>{state==='复杂'?'说不清':state}</button>)}</div>{record.mood&&<details className="mood-more"><summary>补充一下（选填）</summary><div className="record-grid"><Field label="感受强度"><select value={record.mood.intensity??''} onChange={e=>setRecord({...record,mood:{...record.mood!,intensity:(e.target.value||undefined) as typeof record.mood.intensity}})}><option value="">未填写</option><option>轻微</option><option>明显</option><option>强烈</option></select></Field><Field label="想写几句"><Input value={record.mood.description??''} onChange={e=>setRecord({...record,mood:{...record.mood!,description:e.target.value}})}/></Field></div><Field label="诱因"><Input value={record.mood.trigger??''} onChange={e=>setRecord({...record,mood:{...record.mood!,trigger:e.target.value}})}/></Field></details>}</div>
    </Section></div>

    <div className="record-dimension" data-health-tab="menstrual" ref={setSectionRef('menstrual')}><Section title="月经与出血" onClear={record.menstrual?()=>setRecord({...record,menstrual:undefined}):undefined} onAdd={!record.menstrual?()=>setRecord({...record,menstrual:{event:'来了',date}}):undefined}>
    ...
    <div className="record-dimension" data-health-tab="weight" ref={setSectionRef('weight')}><Section title="体重与食欲" onClear={record.weight||record.appetite?()=>setRecord({...record,weight:undefined,appetite:undefined}):undefined} onAdd={!record.weight?()=>setRecord({...record,weight:{direction:'增加',date}}):undefined}>
    ...
    <div className="record-dimension" data-health-tab="exercise" ref={setSectionRef('exercise')}><Section title="运动" onClear={record.exercise?()=>setRecord({...record,exercise:undefined}):undefined} onAdd={!record.exercise?()=>setRecord({...record,exercise:{type:''}}):undefined}>
    ...
    <div className="record-dimension" data-health-tab="diet" ref={setSectionRef('diet')}><Section title="饮食与饮品" onClear={record.diet?()=>setRecord({...record,diet:undefined}):undefined} onAdd={!record.diet?()=>setRecord({...record,diet:{foods:[]}}):undefined}>
