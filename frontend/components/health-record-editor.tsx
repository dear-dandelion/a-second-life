...
export default function HealthRecordEditor({initialDate,onBack}:{initialDate?:string;onBack:()=>void}){
  ...
  const save=async()=>{setSaving(true);try{await services.healthRecords.save(date,preparedRecord({...record,date:date as HealthRecord['date']}));setDates(await services.healthRecords.list());toast.add({title:'健康记录已保存',type:'success'})}catch{toast.add({title:'保存失败，请检查填写内容',type:'error'})}finally{setSaving(false)}};
  ...
  return <div className="record-editor">
    ...
    <div className="record-dimension" data-health-tab="symptoms" ref={setSectionRef('symptoms')}><Section title="身体症状" onAdd={record.symptoms.length?()=>setRecord({...record,symptoms:[...record.symptoms,{symptom:'',occurred:true,quote:'用户手动填写'}]}):undefined} onClear={record.symptoms.length?()=>setRecord({...record,symptoms:[]}):undefined}>
      {(record.symptoms.length?record.symptoms:[{symptom:'',occurred:true,quote:'用户手动填写'}]).map((item,index)=><div className="record-card" key={item.id??index}>
        ...
        <div className="record-grid"><Field label="发生"><select value={String(item.occurred)} onChange={e=>updateSymptom(index,{occurred:e.target.value==='true'})}><option value="true">是</option><option value="false">否</option></select></Field><Field label="程度"><select value={item.severity??''} onChange={e=>updateSymptom(index,{severity:(e.target.value||undefined) as typeof item.severity})}><option value="">未填写</option>{['轻','中','重'].map(x=><option key={x}>{x}</option>)}</select></Field><Field label="次数"><Input type="number" min="0" value={item.frequencyCount??''} onChange={e=>updateSymptom(index,{frequencyCount:numberOrUndefined(e.target.value)})}/></Field><Field label="趋势"><select value={item.trend??''} onChange={e=>updateSymptom(index,{trend:(e.target.value||undefined) as typeof item.trend})}><option value="">未填写</option>{['加重','减轻','稳定'].map(x=><option key={x}>{x}</option>)}</select></Field></div>
    ...
    <div className="record-dimension" data-health-tab="mood" ref={setSectionRef('mood')}><Section title="情绪" onClear={record.mood?()=>setRecord({...record,mood:undefined}):undefined}>
      <div className="record-card mood-record-card"><p className="mood-record-hint">选一个最接近今天感受的状态即可</p><div className="mood-state-options">{(['舒展','平静','低落','焦虑','烦躁','复杂'] as const).map(state=><button type="button" key={state} className={record.mood?.state===state?'active':''} onClick={()=>setRecord({...record,mood:{...record.mood,state,source:'manual'}})}>{state==='复杂'?'说不清':state}</button>)}</div>{record.mood&&<details className="mood-more" open><summary>补充一下（选填）</summary><div className="record-grid"><Field label="感受强度"><select value={record.mood.intensity??''} onChange={e=>setRecord({...record,mood:{...record.mood!,intensity:(e.target.value||undefined) as typeof record.mood.intensity}})}><option value="">未填写</option><option>轻微</option><option>明显</option><option>强烈</option></select></Field><Field label="想写几句"><Input placeholder="可选填" value={record.mood.description??''} onChange={e=>setRecord({...record,mood:{...record.mood!,description:e.target.value}})}/></Field></div><Field label="诱因"><Input placeholder="可选填" value={record.mood.trigger??''} onChange={e=>setRecord({...record,mood:{...record.mood!,trigger:e.target.value}})}/></Field></details>}</div>
    </Section></div>
    ...
    <div className="record-dimension" data-health-tab="weight" ref={setSectionRef('weight')}><Section title="体重与食欲" onClear={record.weight||record.appetite?()=>setRecord({...record,weight:undefined,appetite:undefined}):undefined}>
      <div className="record-card"><div className="record-grid"><Field label="体重方向"><select value={record.weight?.direction??''} onChange={e=>setRecord({...record,weight:{...record.weight,direction:e.target.value as '增加'|'减少',date:record.weight?.date??date}})}><option value="">请选择</option><option>增加</option><option>减少</option></select></Field><Field label="变化量"><Input placeholder="如：2kg（可选填）" value={record.weight?.amount??''} onChange={e=>setRecord({...record,weight:{...record.weight,direction:record.weight?.direction??'增加',date:record.weight?.date??date,amount:e.target.value}})}/></Field><Field label="速度"><select value={record.weight?.speed??''} onChange={e=>setRecord({...record,weight:{...record.weight,direction:record.weight?.direction??'增加',date:record.weight?.date??date,speed:(e.target.value||undefined) as '突然'|'缓慢'|undefined}})}><option value="">可选填</option><option>突然</option><option>缓慢</option></select></Field><Field label="记录日期"><Input type="date" max={today} value={record.weight?.date??''} onChange={e=>setRecord({...record,weight:{...record.weight,direction:record.weight?.direction??'增加',date:e.target.value}})}/></Field><Field label="食欲"><select value={record.appetite??''} onChange={e=>setRecord({...record,appetite:(e.target.value||undefined) as HealthRecord['appetite']})}><option value="">请选择</option>{['增加','减少','正常'].map(x=><option key={x}>{x}</option>)}</select></Field></div></div>
    </Section></div>
    ...
    <div className="record-dimension" data-health-tab="exercise" ref={setSectionRef('exercise')}><Section title="运动" onClear={record.exercise?()=>setRecord({...record,exercise:undefined}):undefined}>
      <div className="record-card record-grid"><Field label="类型"><Input placeholder="如：散步、瑜伽" value={record.exercise?.type??''} onChange={e=>setRecord({...record,exercise:{...record.exercise,type:e.target.value}})}/></Field><Field label="时长"><Input placeholder="如：30分钟（可选填）" value={record.exercise?.duration??''} onChange={e=>setRecord({...record,exercise:{...record.exercise,type:record.exercise?.type??'',duration:e.target.value}})}/></Field><Field label="频率"><Input placeholder="可选填" value={record.exercise?.frequency??''} onChange={e=>setRecord({...record,exercise:{...record.exercise,type:record.exercise?.type??'',frequency:e.target.value}})}/></Field><Field label="强度"><select value={record.exercise?.intensity??''} onChange={e=>setRecord({...record,exercise:{...record.exercise,type:record.exercise?.type??'',intensity:(e.target.value||undefined) as NonNullable<HealthRecord['exercise']>['intensity']}})}><option value="">可选填</option>{['轻松','适中','累'].map(x=><option key={x}>{x}</option>)}</select></Field></div>
    </Section></div>
